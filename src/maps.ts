import { Rule, Warning, first, checkType } from './warnings/common';
...
const map = (keyRule: Rule, valueRule: Rule): Rule =>
  first(checkType(Object), (data, path) => {
    ...
    return keys.reduce((warnings: Warning[], key: string) => warnings.concat(keyRule(key, path)).concat(valueRule(data[key], path.concat(key))), []);
  });
