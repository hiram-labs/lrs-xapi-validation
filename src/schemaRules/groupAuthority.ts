import { composeRules, Rule, optional, restrictToSchema, restrictToCollection } from '../warnings/common';
...
export default composeRules([
  ...
  (data, path) => {
    ...
    const invalidMembers = data.member.filter((member: any) => member.objectType !== 'Agent' && member.objectType !== undefined);
