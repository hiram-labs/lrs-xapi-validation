import { restrictToSchema, restrictToCollection, composeRules, optional, Rule, Warning } from '../warnings/common';
...
import { languageMap, iri, extensions, interactionType, stringValue, interactionComponent } from '../factory';
...
const getUnsupportedComponents = (interactionType: string): string[] => {
  ...
  return allComponents.filter((component) => !supportedComponents.includes(component));
};
...
export default composeRules([
  ...
  (data, path): Warning[] => {
    ...
    const invalidComponents = unsupportedComponents.filter((component) => data[component] !== undefined);
    if (invalidComponents.length > 0) return [new InvalidComponentsWarning(data, path, invalidComponents)];
    return [];
  },
  (data, path): Warning[] => {
    if (!isPlainObject(data)) return [];
    const missingInteractionType = data.correctResponsesPattern !== undefined && data.interactionType === undefined;
    if (missingInteractionType) {
      return [new MissingInteractionTypeWarning(data, path)];
    }
    return [];
  }
]) as Rule;
