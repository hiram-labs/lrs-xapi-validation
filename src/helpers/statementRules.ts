import { composeRules, Rule } from '../warnings/common';
...
export default composeRules([
  (data, path) => {
    ...
    const objectIsActivity = isObject(data.object) && (data.object.objectType === 'Activity' || data.object.objectType === undefined);
    const hasInvalidProps = isObject(data.context) && (data.context.platform !== undefined || data.context.revision !== undefined);
  ...
  (data, path) => {
    ...
    const objectIsStatementRef = data.object && data.object.objectType === 'StatementRef';
